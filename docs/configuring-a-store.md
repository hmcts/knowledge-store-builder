...
| `KSB_SENSITIVE_PATTERNS` | case reference, email address, National Insurance number, UK postcode | Extra rules for mined commit text that must not be stored, as a JSON object of rule name → regex merged over the defaults: `KSB_SENSITIVE_PATTERNS='{"listing-reference": "\\bREF/[0-9]{4}\\b"}'`. A value matching any rule is dropped whole and counted in the run report. Malformed JSON raises rather than emptying the rules — see [Withholding text that identifies a case](how-it-works.md#withholding-text-that-identifies-a-case) |
