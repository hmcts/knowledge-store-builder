...
def _namespaced_id(
    ...
    Split out of `merge_nodes` because Sonar measured that function's cognitive
    complexity at 17 against a limit of 15 once #115 added the basis comparison.
    Behaviour is unchanged: the stem is the source file's when there is one and the
    label's otherwise, and an empty stem leaves the original id alone.
    """
    ...
    # differently. Computed on every run, including the default, so a store learns
...
def _free_id(candidate: str, taken: dict) -> str:
    """`candidate` with the lowest numeric suffix not already in `taken`.

    Called only where `candidate` is known to be taken, which is why the first
    suffix tried is 2 and there is no pass-through branch: a guard for a case the
    single call site has already excluded would be a branch nothing can reach.

    The collision this resolves is the one the namespacing exists to prevent,
    reappearing in the step that resolves it: two identities differing only in
    label share `stem` and `original`, so the namespaced form collides and a plain
    assignment drops one. That lost 13 of 47,653 nodes on a real estate while
    `namespaced` reported 367 kept apart - a counter overstating success, which is
    the reassuring direction and so the dangerous one.

    The search advances rather than stopping at `_2`, because a third identity
    sharing the same stem and original would otherwise take the second one's name
    and drop it: the same loss, one collision later.
    """
    suffix = 2
...
def merge_nodes(
    ...
    **It resolves through the identity, not through the original id.** The first
    ...
    for identity, node in by_identity.items():
        ...
        # namespaced form collides and a plain assignment drops one. That is the very
        # collision the namespacing exists to resolve, reintroduced by the step that
        # resolves it; `_free_id` carries what it cost.
        if node["id"] in nodes:
