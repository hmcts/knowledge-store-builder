...
def _language_step_definitions(repo_dir: Path, language: dict) -> dict[str, tuple[str, str]]:
...
class Feature(TypedDict):
    """A parsed .feature file, accumulated line by line."""
...
def parse_feature(path: Path, repo_dir: Path) -> Feature | None:
    feature: Feature = {
        "rel": str(path.relative_to(repo_dir)),
        "name": None,
        "scenarios": [],
...
class GraphEnricher:
    ...
    def __init__(self, graph: dict, labels: dict[str, str]) -> None:
        ...
        self.max_community = max((n.get("community", 0) for n in graph["nodes"]), default=0)
    ...
    def add_scenarios(
        self, feature: Feature, feature_id: str, repo: str, stem: str, community: int
    ) -> None:
        rel = feature["rel"]
    ...
    def add_stepdef_edges(
        self,
        feature: Feature,
        feature_id: str,
        step_index: dict[str, tuple[str, str]],
        class_node_by_file: dict[str, str],
    ) -> None:
        matched_files = {step_index[step][1] for step in feature["steps"] if step in step_index}
    ...
    def add_ticket_edges(
        self, feature: Feature, feature_id: str, repo: str, community: int
    ) -> None:
        rel = feature["rel"]
        for ticket in sorted(feature["tickets"]):
            ticket_id = self.ticket_ids.setdefault(ticket, f"jira::{norm_id(ticket)}")
            if self.add_node(ticket_id, ticket, repo, rel, community, {"kind": kinds.TICKET}):
    ...
    def add_feature(
        ...
        created = self.add_node(
            feature_id,
            # parse_feature rejects unnamed features, so this holds; the
            # fallback keeps the node labelled if that ever changes
            feature["name"] or Path(rel).stem,
            repo,
            rel,
            community,
            {
                ...
                "tickets": sorted(feature["tickets"]),
...
def enrich_repository(repo_dir: Path, enricher: GraphEnricher) -> None:
    features = [f for f in sorted(repo_dir.rglob("*.feature")) if "node_modules" not in f.parts]
    if not features:
        return
    repo = repo_dir.name
